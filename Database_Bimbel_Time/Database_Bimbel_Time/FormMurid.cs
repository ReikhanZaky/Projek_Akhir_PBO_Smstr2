...
{
    public partial class FormMurid : Form
    {
        private User user;

        public FormMurid(User u)
        {
            InitializeComponent();
            this.user = u;
            this.Load += FormMurid_Load;
        }
     
        private void FormMurid_Load(object sender, EventArgs e)
        {
        ...
        private void LoadJadwal()
        {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                JOIN MuridKelas mk ON mk.KelasID = k.KelasID
                JOIN Murid m ON mk.MuridID = m.MuridID
                ...
                using (var cmd = new NpgsqlCommand(query, conn))
                {
                    ...
                    using (var adapter = new NpgsqlDataAdapter(cmd))
                    {
                        ...
                        dgvJadwal.DataSource = dt; // pastikan dgvJadwal sudah dibuat
                    }
                }
            }
        }
        ...
        {
            TampilkanKonten(new UCJadwalMurid(user));
        }


        private void btnProfil_Click(object sender, EventArgs e)
        {
            TampilkanKonten(new UCProfilMurid(user)); 
        }
